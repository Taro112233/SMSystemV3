// components/ProductsManagement2/index.tsx
// ProductsManagement2 - Minimal redesign with pagination
...
interface ProductsManagement2Props {
  orgSlug: string;
  userRole: string;
}
...
export default function ProductsManagement2({
  orgSlug,
  userRole,
}: ProductsManagement2Props) {
  ...
  const canManage: boolean = Boolean(userRole && ['ADMIN', 'OWNER'].includes(userRole));
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      // Update total from API response (ถ้า API ส่งมา)
  ...
  const handleCreateClick = () => {
    if (!canManage) {
      toast.error('ไม่มีสิทธิ์', {
        description: 'เฉพาะ ADMIN และ OWNER เท่านั้น',
      });
      return;
    }
